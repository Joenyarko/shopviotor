...
import { Plus, Edit2, Trash2, RefreshCw, X, AlertCircle, Layers, Image as ImageIcon, Search } from 'lucide-react';
...
const AdminBrands = () => {
  ...
  const paginatedBrands = filteredBrands.slice((page - 1) * itemsPerPage, page * itemsPerPage);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-secondary-900 dark:text-white">Brand Management</h2>
              ...
              <tbody className="divide-y divide-secondary-100 dark:divide-secondary-800">
                {paginatedBrands.map((brand) => (
                  <tr key={brand.uuid} className="hover:bg-secondary-50/50 dark:hover:bg-secondary-800/30">
                    ...
                    <td className="p-4 text-secondary-500 dark:text-secondary-400 hidden md:table-cell truncate max-w-xs">{brand.description || '—'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-secondary-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-secondary-900 border border-secondary-200 dark:border-secondary-800 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-secondary-500 dark:text-secondary-400 uppercase mb-1.5">Brand Name</label>
